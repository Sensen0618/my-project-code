...
tar = 4;         % 4 targets  
rad = 4;         % 4 radars
Psize = 5000;    % Number of populations(k=1,3)
% Psize = 182000;  %(k=2)
...
P = (10000*ones(rad,tar,Psize)+repmat(unidrnd(10000,rad,1,Psize),[1 tar])).*[0 1 0 0]';   % Power matrix
% a = sum(P,2);
A = [2581.65 2527.14 2527.14 2581.65]'.*ones(4,4,Psize);       
% v_min = -200;
% v_max = 200;           % Speed limit of power particles
v_min = -120;
v_max = 120; 
vv = v_min + rand(rad,1,Psize)*(v_max - v_min);
v = repmat(vv,[1 tar]).*[0 1 0 0]';
% v1_min = -100;
% v1_max = 100;          % Velocity limitation of aperture particles
% v1 = v1_min + rand(rad,tar,Psize)*(v1_max - v1_min);
P_min = 0;
% P_max = 90000000000000;         % Power constraint
P_max = 10000000; 
A_min = 0;
...
% The detection probability of each individual radar for each target
