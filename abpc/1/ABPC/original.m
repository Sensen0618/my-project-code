...
%disp(valid_matrices); 
%%
P = [60000*ones(rad,tar,ssize)+unidrnd(60000,rad,tar,ssize)];
for i=1:24
    P1 = [60000*ones(rad,tar,ssize)+unidrnd(60000,rad,tar,ssize)].*valid_matrices(:,:,i);   %功率矩阵
    P = cat(3,P,P1);
end
a = sum(P,2);
...
v_min = -400;
v_max = 400;           % 功率粒子的速度限制
v = [v_min + rand(rad,tar,Psize)*(v_max - v_min)];
% v1_min = -100;
...
SNR = (P.*A.^2.*lamda.^2.*sigma)./(4^3*pi*L*k*F*B*T.*R.^4.*10^12);   %信噪比
