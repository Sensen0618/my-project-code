...
P = (200000000*ones(rad,tar,Psize)+repmat(unidrnd(200000000,rad,1,Psize),[1 tar])).*[0 1 1 1]';   % power matrix
% a = sum(P,2);
A = [2597.21 2482.81 2542.91 2539.21]'.*ones(4,4,Psize);       % update array element matrix from power matrix
v_min = -2000;
v_max = 2000;           % velocity limit for power particles
vv = v_min + rand(rad,1,Psize)*(v_max - v_min);
v = repmat(vv,[1 tar]).*[0 1 1 1]';
% v1_min = -100;
...
SNR = (P.*A.*lamda.^2.*sigma)./(4^3*pi.^3*L*k*F*B*T.*R.^4.*10^12);   % SNR
